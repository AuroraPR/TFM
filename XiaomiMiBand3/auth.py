...
class MiBand3(Peripheral):
    ...
    def get_steps(self):
        ...
        a = char.read()
        steps = struct.unpack('h', a[1:3])[0] if len(a) >= 3 else 0
        meters = struct.unpack('h', a[5:7])[0] if len(a) >= 7 else 0
        fat_gramms = struct.unpack('h', a[2:4])[0] if len(a) >= 4 else 0
        # why only 1 byte??
        callories = struct.unpack('b', a[9])[0] if len(a) >= 10 else 0
        JsonData= {
            "steps": steps,
            "meters": meters,
